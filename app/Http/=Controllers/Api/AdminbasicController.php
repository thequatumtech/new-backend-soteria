...
use App\Models\InsurancePlanModels\{HomePlan,LifePlan,CriticalIllnessPlan,PersonalAccidentPlan,InPatientPlan,InOutPatientPlan,PetPlan,DentalPlan,TravelPlan,MarinePlan};
    ...
    public function getCities(Request $request)
    {
        try {
            if ($request->country_id) {
                $data=Cities::where('country_id',$request->country_id)->get();
            }
            else
            {
                $data=Cities::all();
    ...
    public function insuranceLimit(Request $request)
    {
        try {
            ...
            elseif ($request->insurance_type==3) {
                ...
                foreach ($plan as $val) {
                    $data[] = [
                        'limit' => (float)$val['normalized_limit'], // Ensures response as a float
                    ];
                }
            }
            elseif ($request->insurance_type==4) {
                ...
                foreach ($plan as $val) {
                    $plan_name = CriticalIllnessPlan::getCriticalIllnessInsurancePlanByLimit($val['limit']);
                    $data[] = [
                        'limit' => (int)$val['limit'],
                        'plan_name' => $plan_name
                    ];
            ...
            elseif ($request->insurance_type==5) {
                ...
                foreach ($plan as $val) {
                    // $period = PersonalAccidentPlan::getPersonalAccidentInsurancePeriodsPlanByLimit($val['limit']);
                    $data[] = [
                        'limit' => (int)$val['limit'],
                    ];
                }
            ...
            elseif ($request->insurance_type==6) {
                ...
                foreach ($plan as $val) {
                    $plan_name = InPatientPlan::getInPatientInsurancePlanByLimit($val['limit']);
                    $data[] = [
                        'limit' => (int)$val['limit'],
                        'plan_name' => $plan_name
                    ];
            ...
            elseif ($request->insurance_type==7) {
                ...
                foreach ($plan as $val) {
                    $plan_name = InOutPatientPlan::getInOutPatientInsurancePlanByLimit($val['limit']);
                    $data[] = [
                        'limit' => (int)$val['limit'],
                        'plan_name' => $plan_name
                    ];
            ...
            elseif ($request->insurance_type==8) {
                ...
                foreach ($plan as $val) {
                    $plan_name = PetPlan::getPetInsurancePlanByLimit($val['limit']);
                    $data[] = [
                        'limit' => (int)$val['limit'],
                        'plan_name' => $plan_name
                    ];
            ...
            elseif ($request->insurance_type==9) {
                ...
                foreach ($plan as $val) {
                    $plan_name = DentalPlan::getDentalInsurancePlanByLimit($val['limit']);
                    $data[] = [
                        'limit' => (int)$val['limit'],
                        'plan_name' => $plan_name
                    ];
            ...
            elseif ($request->insurance_type==10) {
                ...
                $data = [];
                    $plan_name = TravelPlan::getTravelInsurancePlanByLimit(0);
                    foreach ($plan_name as $val) {
                        $data[] = [
            ...
            elseif ($request->insurance_type==11) {
                $plan=MarinePlan::getMarinePlanLimit($request);
                $data = [];
            ...
            return response()->json(['status' => true, 'status_code' => 200, 'message' => 'Get Insurance Limit successfully','data' => $data]);
        }
        catch(\Exception $e)
        {
            return response()->json(['status' => false, 'status_code' => 500, 'message' => $e->getMessage().' '.$e->getFile().' '.$e->getLine(), 'data' => array()]);
