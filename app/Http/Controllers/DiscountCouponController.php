...
class DiscountCouponController extends Controller
{
        ...
        if ($request->form_type == 'add') {
            ...
            if ($request->send_coupon_to) {
                ...
                foreach ($pending_clients as $client_coupon_record) {
                    ...
                    if ($found) {
                        Mail::to($found->email_id)->queue(new SendCoupon($client_coupon_record));
                    }
            ...
            if ($request->hasFile($file)) {
              ...
              ClientDiscountCoupon::where('coupon_id', $coupon->id)->update([
                'insurance_company_id' => $coupon->insurance_company_id,
                'line_of_business_id' => $coupon->line_of_business_id,
                'percentage' => $coupon->percentage,
                'effective_date' => $coupon->effective_date,
                'expiry_date' => $coupon->expiry_date,
                'description' => $coupon->description,
                'send_to' => $coupon->send_to ?? '0',
                'attachment' => $coupon->attachment,
            ]);
            if ($request->send_coupon_to) {
                if ($request->selected_clients) {
                    $client_ids = explode(',', $request->selected_clients);
    ...
    public function send_coupon(Request $request)
    {
        ...
        $pending_clients = ClientDiscountCoupon::where('coupon_id', $coupon->id)->where('send_to', '1')->where('is_sent', '0')->where('status', '1')->get();
