...
class LifePlanController extends Controller
{
    ...
    public function edit_life_plan(Request $request, $id)
    {
        ...
        $occupations = Occupations::all();
        $chronics = ChronicDisease::all();
        return view('admin.plan.life-plans.edit_life_plan',compact('insurance_companies','line_of_businesses','countries','cities','districts','ages','plan','occupations','chronics','selected_country_ids','selected_city_ids','selected_district_ids'));
    }
    ...
    public function save_life_plan(Request $request)
    {
        if($request->form_type == 'add'){
            ...
            $requestCoverIds = [];
             ...
             LifePlanPolicyCover::where('life_plan_id', $plan_id)
            ...
            foreach ($request->policy_covers as $single) {
                ...
                // $life_insurance_plan_policy_covers->save();
                 if (!empty($single['id'])) {
                    $cover = LifePlanPolicyCover::find($single['id']);
                    if ($cover) {
                        $cover->update([
                    ...
                    LifePlanPolicyCover::create([
                        'life_plan_id' => $plan_id,
                        'cover_name' => $single['cover_name'],
                         'cover_limit' => str_replace(',', '', $single['cover_limit']),
                    ]);
                }
