...
class TravelPlan extends Model
{
    ...
    public function discount_bands()
    {
        return $this->hasMany(TravelPlanDiscountBand::class, 'travel_plan_id', 'id');
    }
    ...
    {
        $plan = TravelPlan::select('plan_name', 'policy_period');
        if ($limit != 0) {
            $plan->where('limit', $limit);
        }
        $plan = $plan->groupBy('plan_name', 'policy_period')->get();
        return $plan;
    }
}
