...
@section('content')
    <main class="flex-grow-1 pt-5">
        <div class="container-fluid">
            <div class="header d-flex justify-content-between p-3 py-2 align-items-center">
                <div class="text-white group-title fw-bold"> {{__('messages.vehicle_category.vehicle_category')}} </div>
            ...
            <div class="d-flex p-3 py-2">
                <a href="{{route('categories.sample.csv')}}" class="ms-auto">
            ...
            <div class="body bg-white py-4 px-4 d-flex flex-column gap-3 pb-5">
                <table id="example" class="table table-striped " style="width:100%">
                    ...
                    <tr>
                        ...
                        <th width="45%"> {{__('messages.vehicle_category.vehicle_brand')}} </th>
                        <th width="45%"> {{__('messages.vehicle_category.vehicle_category')}} </th>
                        <th class="no-order" width="10%"></th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach($all_categories as $key=> $single)
                        @if(!$single->trashed())
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{isset($single->vehicleBrand->name)&&!empty($single->vehicleBrand->name)?$single->vehicleBrand->name:'-'}}</td>
                                <td>{{$single->name}}</td>
                                <td>
                                    <div class="d-flex gap-3 justify-content-evenly align-items-center px-2">
                                        <button class="btn p-0 m-0 editbtn" value="{{$single->id}}" data-name="{{$single->name}}" data-brand="{{$single->vehicle_brand_id}}" data-bs-toggle="modal" data-bs-target="#editModal">
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <div class="modal-header text-white " style="background-color: #104E9E;">
                    <h1 class="modal-title fs-5"> {{__('messages.vehicle_category.add_vehicle_category')}} </h1>
                    <button data-bs-dismiss="modal" class="btn">
                        <img src="{{asset('img/icon-close.svg')}}" alt="">
                    </button>
                </div>
                <form action="{{route('vehicle-categories.create')}}" id= "add" method="post">
                    @csrf
                            ...
                            <div class="container">
                                <div class="row">
                                    <div class="col-12 col-lg-6 pt-2">
                                        <div> {{__('messages.vehicle_category.vehicle_brand')}} </div>
                                        <select name="vehicle_brand_id" class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height: 3.5rem;" required>
                                            <option value="" disabled hidden>--Select--</option>
                                            @foreach($all_brands as $single)
                                                <option value="{{$single->id}}">{{$single->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-12 col-lg-6 pt-2">
                                        <div> {{__('messages.vehicle_category.add_vehicle_category')}} </div>
                                        <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                            <input type="text" required name="name" style="border: none;" maxlength="100">
                                        </div>
                                    </div>
                                </div>
                                <div class="row pt-5">
    ...
    <div class="modal fade " id="editModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <div class="modal-header text-white " style="background-color: #104E9E;">
                    <h1 class="modal-title fs-5"> {{__('messages.vehicle_category.edit_vehicle_category')}} </h1>
                    <button data-bs-dismiss="modal" class="btn">
                        <img src="{{asset('img/icon-close.svg')}}" alt="">
                    </button>
                </div>

                <form action="{{route('vehicle-categories.update')}}" id="edit_age" method="post">
                    @csrf
                    <input type="hidden" id="vehicle_category_id" name="vehicle_category_id" style="border: none;" value="">
                    <div class="modal-body">
                        <div class="row p-3" style="color: #92959A;">
                            <div class="container">
                                <div class="row">
                                    <div class="col-12 col-lg-6 pt-2">
                                        <div> {{__('messages.vehicle_category.vehicle_brand')}} </div>
                                        <select name="vehicle_brand_id" id="edit_vehicle_brand_id" class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height: 3.5rem;" required>
                                            <option value="" disabled hidden>--Select--</option>
                                            @foreach($all_brands as $single)
                                                <option value="{{$single->id}}">{{$single->name}}</option>
                                            @endforeach
                                    ...
                                    <div class="col-12 col-lg-6 pt-2">
                                        <div>{{__('messages.vehicle_category.vehicle_category')}}</div>
                                        <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                            <input type="text" required name="name" id="name_edit" style="border: none;" maxlength="100">
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="modal fade " id="DeleteModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <div class="modal-header text-white " style="background-color: #104E9E;">
                    <h1 class="modal-title fs-5"> {{__('messages.vehicle_category.delete_vehicle_category')}} </h1>
                    <button data-bs-dismiss="modal" class="btn">
                        <img src="{{asset('img/icon-close.svg')}}" alt="">
                    </button>
                </div>
                            ...
                            <div class="container">
                                <div class="row pt-5">
                                    <div class="col-12 col-lg-9">
                                        ...
                                        <input type="hidden" id="deleteing_id" name="vehicle_category_id">
                                    </div>
                                </div>
                                <div class="col-12 col-lg-3">
                                    <div class="pt-4 " style="border: none;">
    ...
    <div class="modal fade " id="addcsvdistrict" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <form action="{{route('category.csv')}}" id= "addcsv" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row p-3" style="color: #92959A;">
                            <div class="container">
...
@section('script')
    ...
    <script>
        ...
        $(document).on('click', '.editbtn', function() {
            ...
            var vehicle_brand_id = $(this).attr("data-brand");
            let edit_vehicle_brand_id = '#edit_vehicle_brand_id option[value="'+vehicle_brand_id+'"]';
            $(edit_vehicle_brand_id).attr("selected", "selected");
