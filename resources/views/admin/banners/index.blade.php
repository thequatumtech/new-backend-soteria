...
<main class="flex-grow-1 pt-5">
    <div class="container-fluid">
        <div class="header d-flex justify-content-between p-3 py-2 align-items-center">
            <div class="text-white group-title fw-bold">{{ __('messages.banner_management.banners') }}</div>
        ...
        <div class="body bg-white py-4 px-4 d-flex flex-column gap-3 pb-5">
            <table id="example" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        <th class="no-show">{{ __('messages.banner_management.sr') }}</th>
                        <th width="40%">{{ __('messages.banner_management.banner_title_link') }}</th>
                        <th width="20%">{{ __('messages.banner_management.image_video') }}</th>
                        <th width="20%">{{ __('messages.banner_management.status') }}</th>
                        <th width="20%">{{ __('messages.banner_management.run_time') }}</th>
                ...
                <tbody>
                    ...
                    <tr>
                            ...
                            @if($banner->image)
                                ...
                                @if(in_array($extension, $videoExtensions))
                                    <video width="120" height="80" controls>
                                        ...
                                        {{ __('messages.banner_management.no_media') }}
                                    </video>
                                @else
                        ...
                        <td>
                            @if($banner->is_active)
                                <span class="badge bg-success">{{ __('messages.banner_management.active') }}</span>
                            @else
                                <span class="badge bg-danger">{{ __('messages.banner_management.inactive') }}</span>
                            @endif
                        </td>
                        <td>
                            {{ $banner->runtime }} {{ __('messages.banner_management.sec') }}
                        </td>
                        <td>
                            <div class="d-flex gap-3 justify-content-evenly align-items-center px-2">
                                <button class="btn p-0 m-0 editbtn"
                                    value="{{$banner->id}}"
                                    data-val="{{$banner->title}}"
                                    data-link="{{ $banner->redirect_url }}"
                                    data-image="{{ asset($banner->image) }}"
                                    data-status="{{ $banner->is_active }}"
                                    data-runtime="{{ $banner->runtime }}"
                                    data-bs-toggle="modal"
                                    data-bs-target="#editModal">
                                    <img src="{{asset('img/icon-edit.png')}}" alt="">
                                </button>
                                <button class="btn p-0 m-0 deletebtn" value="{{$banner->id}}">
    ...
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header text-white" style="background-color: #104E9E;">
                <h1 class="modal-title fs-5">{{ __('messages.banner_management.add_banner') }}</h1>
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="col-12 col-lg-4 pt-2">
                                    <div>{{ __('messages.banner_management.banner_title') }}</div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <input type="text" name="name" style="border: none;" maxlength="50" required>
                                    </div>
                                </div>
                                <div class="col-12 col-lg-4 pt-2">
                                    <div>{{ __('messages.banner_management.banner_upload') }}</div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <input type="file" name="image" style="border: none;" accept="image/*,video/*" required>
                                    </div>
                                </div>
                                <div class="col-12 col-lg-4 pt-2">
                                    <div>{{ __('messages.banner_management.banner_link') }}</div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                    ...
                                    <div id="add_link_error" class="text-danger mt-1" style="display:none; font-size:12px;">
                                        {{ __('messages.banner_management.link_error') }}
                                    </div>
                                </div>
                                <div class="col-12 col-lg-4 pt-2">
                                    <div>{{ __('messages.banner_management.run_time') }}</div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <select name="runtime" class="form-control" style="border: none;" required>
                                            @for($i = 5; $i <= 60; $i += 5)
                                                <option value="{{ $i }}">{{ $i }} {{ __('messages.banner_management.sec') }}</option>
                                            @endfor
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row pt-5">
                                ...
                                <div class="col-12 col-lg-3">
                                    <div class="pt-4" style="border: none;">
                                        <button type="submit" class="btn rounded-1 w-100 text-white opacity-50 p-2" style="background-color: #EF7C00;">
                                            {{ __('messages.age.add') }}
                                        </button>
                                    </div>
                                </div>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header text-white" style="background-color: #104E9E;">
                <h1 class="modal-title fs-5">{{ __('messages.banner_management.edit_banner') }}</h1>
                <button data-bs-dismiss="modal" class="btn">
                    <img src="{{asset('img/icon-close.svg')}}" alt="">
                </button>
            </div>
                        ...
                        <div class="container">
                            <div class="row">
                                ...
                                <div class="col-12 col-lg-6 pt-2">
                                    <div>{{ __('messages.banner_management.banner_upload') }}</div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <input type="file" id="image_edit_input" name="image" style="border: none;" accept="image/*,video/*">
                                        <div class="mt-2">
                                            <img id="banner_edit" src="" width="120" style="display:none;">
                                            <video id="banner_video_edit" width="200" controls style="display:none;">
                                ...
                                <div class="col-12 col-lg-6 pt-2">
                                    <div>{{ __('messages.banner_management.status') }}</div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <select id="status_edit" name="status" class="form-control" style="border: none;">
                                            <option value="1">{{ __('messages.banner_management.active') }}</option>
                                            <option value="0">{{ __('messages.banner_management.inactive') }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-12 col-lg-6 pt-2">
                                ...
                                <div class="col-12 col-lg-6 pt-2">
                                    <div>{{ __('messages.banner_management.run_time') }}</div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <select id="runtime_edit" name="runtime" class="form-control" style="border: none;" required>
                                            @for($i = 5; $i <= 60; $i += 5)
                                                <option value="{{ $i }}">{{ $i }} {{ __('messages.banner_management.sec') }}</option>
                                            @endfor
                                        </select>
...
<div class="modal fade" id="DeleteModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header text-white" style="background-color: #104E9E;">
                <h1 class="modal-title fs-5">{{ __('messages.banner_management.delete_banner') }}</h1>
                <button data-bs-dismiss="modal" class="btn">
                    <img src="{{asset('img/icon-close.svg')}}" alt="">
                </button>
            </div>
                        ...
                        <div class="container">
                            <div class="row pt-5">
                                <div class="col-12 col-lg-9">
                                    <h4>{{ __('messages.banner_management.confirm_delete') }}</h4>
...
<script>
    ...
    $(document).on('click', '.editbtn', function() {
        ...
        var runtime = $(this).data('runtime') || 10;

        $("#cd_id").val(cd_id);
        $("#name_edit").val(name);
        $("#link_edit").val(link);
        $("#runtime_edit").val(runtime);
        $('#edit_link_error').hide();
