...
<main class="flex-grow-1 pt-5">
    <div class="container-fluid">
                ...
                <div class="row p-3">
                    <div class="container">
                        <div class="row">
                            ...
                            <label for="grouptype">{{__('messages.cancelled_by_admin_report.group_report_by')}}</label>
                                <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height: 3.5rem;" name="grouptype" id="grouptype" onchange="toggleDateFields()">
                                    <option value="insurance_company" {{ request('grouptype')=='insurance_company' ? 'selected' : ''}}>{{__('messages.cancelled_by_admin_report.grouptype_insurance_company')}}</option>
                                    <option value="policy_type" {{ request('grouptype')=='policy_type' ? 'selected' : ''}}>{{__('messages.cancelled_by_admin_report.grouptype_policy_type')}}</option>
                                    <option value="blacklisted" {{ request('grouptype')=='blacklisted' ? 'selected' : ''}}>{{__('messages.cancelled_by_admin_report.grouptype_blacklisted')}}</option>
                                </select>
                            </div>
                            <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                            <label for="insurance_company">{{__('messages.cancelled_by_admin_report.insurance_company')}}</label>
                                ...
                                <option value="">{{__('messages.cancelled_by_admin_report.select_company')}}</option>
                            ...
                            <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                              <label for="blacklisted">{{__('messages.cancelled_by_admin_report.blacklisted')}}</label>
                                <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height: 3.5rem;" name="client_name" id="blacklisted">
                                        <option value="">{{__('messages.cancelled_by_admin_report.select_blacklisted')}}</option>
                            ...
                            <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                                <button type="submit" class="btn rounded-1 text-white opacity-50 p-2" style="background-color: #EF7C00;">{{__('messages.cancelled_by_admin_report.generate_report')}}</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="m-2 text-center">
            ...
            <h4>{{__('messages.cancelled_by_admin_report.policies_by')}}
    @php
    switch(request('grouptype'))
    {
    case 'policy_type':
    echo __('messages.cancelled_by_admin_report.heading_policy_type');
    break;
    case 'blacklisted':
    echo __('messages.cancelled_by_admin_report.heading_blacklisted');
    break;
    case 'insurance_company':
    echo __('messages.cancelled_by_admin_report.heading_insurance_company');
    break;
    default:
    echo __('messages.cancelled_by_admin_report.heading_insurance_company');
    break;
...
<p>{{__('messages.cancelled_by_admin_report.from')}} {{ request('issue_date') ?? __('messages.cancelled_by_admin_report.no_date') }} {{__('messages.cancelled_by_admin_report.to')}} {{ request('expiry_date') ?? __('messages.cancelled_by_admin_report.no_date') }}</p>
        </div>
        <div class="body bg-white py-4 px-4 d-flex flex-column gap-3 pb-5">
            @if($policies)
            ...
            <table class="table table-striped " style="width:100%">
                ...
                <tbody>
                    ...
                    <!-- Totals Row -->
                    <tr>
        <td colspan="6"><strong>{{__('messages.cancelled_by_admin_report.total')}}</strong></td>
            ...
            @endforeach
         @else
<h4 class="text-center">{{__('messages.cancelled_by_admin_report.no_record')}}</h4>
@endif
        </div>
    </div>
    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
