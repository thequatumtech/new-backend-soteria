...
<main class="flex-grow-1 pt-5">
    <div class="container-fluid">
        <div class="header d-flex justify-content-between p-3 py-2 align-items-center">
            <div class="text-white group-title fw-bold">{{__('messages.reports.sold_policy_report')}} {{__('messages.reports.reports')}}</div>
            @include('admin.reports.partials.report-download-controls', [
            'pdfRoute' => route('reports.download', ['report' => 'sold_policy_report', 'format' => 'pdf']),
            'excelRoute' => route('reports.download', ['report' => 'sold_policy_report', 'format' => 'excel']),
            'filePrefix' => 'Sold_Policy_Report',
            ])
        </div>
        <!-- Form to be toggled -->
                    ...
                    <div class="container">
                        <div class="row">
                           <div class="col-12 col-lg-12 pt-4 d-flex flex-column">
                                <label for="grouptype">{{ __('messages.sold_policy_report.group_by') }}</label>
                                <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height:3.5rem;" name="grouptype" id="grouptype" onchange="toggleDateFields()">
                                    <option value="insurance_company" {{ request('grouptype') == 'insurance_company' ? 'selected' : '' }}>
                                        {{ __('messages.sold_policy_report.insurance_company') }}
                                    </option>
                                    <option value="agent_name" {{ request('grouptype') == 'agent_name' ? 'selected' : '' }}>
                                        {{ __('messages.sold_policy_report.agent') }}
                                    </option>
                                    <option value="supervisor_name" {{ request('grouptype') == 'supervisor_name' ? 'selected' : '' }}>
                                        {{ __('messages.sold_policy_report.supervisor') }}
                                    </option>
                                    <option value="policy_type" {{ request('grouptype') == 'policy_type' ? 'selected' : '' }}>
                                        {{ __('messages.sold_policy_report.policy_type') }}
                                    </option>
                                    <option value="client_name" {{ request('grouptype') == 'client_name' ? 'selected' : '' }}>
                                        {{ __('messages.sold_policy_report.client_name') }}
                                    </option>
                                    <option value="position" {{ request('grouptype') == 'position' ? 'selected' : '' }}>
                                        {{ __('messages.sold_policy_report.position') }}
                                    </option>
                                </select>
                            </div>
                           <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                                <label for="insurance_company">{{ __('messages.sold_policy_report.insurance_company') }}</label>
                                <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height:3.5rem;" name="insurance_company" id="insurance_company">
                                    <option value="">{{ __('messages.sold_policy_report.select_company') }}</option>
                                    @foreach ($insuranceCompanies as $company)
                                        <option value="{{ $company->id }}" {{ request('insurance_company') == $company->id ? 'selected' : '' }}>
                        ...
                        <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                            ...
                            <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                                <label for="client_name">{{ __('messages.sold_policy_report.clients_name') }}</label>
                                <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height:3.5rem;" name="client_name" id="client_name">
                                    <option value="">{{ __('messages.sold_policy_report.select_client') }}</option>
                                    @foreach ($clients as $client)
                                        <option value="{{ $client->id }}" {{ request('client_name') == $client->id ? 'selected' : '' }}>
                                            {{ $client->first_name }} {{ $client->father_name }} {{ $client->surname }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                           <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                                <label for="client_position">{{ __('messages.sold_policy_report.client_position') }}</label>
                                <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height:3.5rem;" name="client_position" id="client_position">
                                    <option value="">{{ __('messages.sold_policy_report.select_position') }}</option>
                                    @foreach ($positions as $position)
                                        <option value="{{ $position->id }}" {{ request('client_position') == $position->id ? 'selected' : '' }}>
                                            {{ $position->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                                <label for="policy_type">{{ __('messages.sold_policy_report.policy_type') }}</label>
                                <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height:3.5rem;" name="policy_type" id="policy_type">
                                    <option value="">{{ __('messages.sold_policy_report.select_policy_type') }}</option>
                                    @foreach ($policyTypes as $type)
                                        <option value="{{ $type['id'] }}" {{ request('policy_type') == $type['id'] ? 'selected' : '' }}>
                                            {{ $type['name'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                                <label for="supervisor">{{ __('messages.sold_policy_report.supervisor_name') }}</label>
                                <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height:3.5rem;" name="supervisor" id="supervisor">
                                    <option value="">{{ __('messages.sold_policy_report.select_supervisor') }}</option>
                            ...
                            <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                                <button type="submit" class="btn rounded-1 text-white opacity-50 p-2" style="background-color:#EF7C00;">
                                    {{ __('messages.sold_policy_report.generate_report') }}
                                </button>
                            </div>                        
                        </div>
                    </div>
        ...
        <div class="m-2 text-center">
            ...
            <h4>
                 {{ __('messages.sold_policy_report.sales_by') }}
                @php
                    $grouptype = request('grouptype');
                    $labelMap  = [
                        'insurance_company' => __('messages.sold_policy_report.sales_by_ins_company'),
                        'agent_name'        => __('messages.sold_policy_report.sales_by_agent'),
                        'policy_type'       => __('messages.sold_policy_report.sales_by_policy_type'),
                        'date'              => __('messages.sold_policy_report.sales_by_date'),
                        'client_name'       => __('messages.sold_policy_report.sales_by_client'),
                        'position'          => __('messages.sold_policy_report.sales_by_client_pos'),
                        'supervisor_name'   => __('messages.sold_policy_report.sales_by_supervisor'),
                    ];
                    echo $labelMap[$grouptype] ?? __('messages.sold_policy_report.sales_by_ins_company');
                @endphp
            </h4>
             <p>
                {{ __('messages.sold_policy_report.from') }} {{ request('issue_date') ?? __('messages.sold_policy_report.no_date') }}
                {{ __('messages.sold_policy_report.to') }}   {{ request('expiry_date') ?? __('messages.sold_policy_report.no_date') }}
            </p>        
        </div>
        <div class="body bg-white py-4 px-4 d-flex flex-column gap-3 pb-5">
            @if ($policies)
            ...
            <table class="table table-striped " style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="no-order no-search">{{__('messages.reports.policy_limit')}}</th>
                ...
                <tbody>
                    ...
                    <!-- Totals Row -->
                    <tr>
                        <td colspan="10"><strong>{{ __('messages.sold_policy_report.total') }}</strong></td>
            ...
            @endforeach
            @else
                            <h4 class="text-center">{{ __('messages.sold_policy_report.no_record') }}</h4>
