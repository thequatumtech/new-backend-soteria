...
<main class="flex-grow-1 pt-5">
    <div class="container-fluid">
        <div class="header d-flex justify-content-between p-3 py-2 align-items-center">
            ...
            'pdfRoute' => route('reports.download', ['report' => 'claims_report', 'format' => 'pdf']),
            'excelRoute' => route('reports.download', ['report' => 'claims_report', 'format' => 'excel']),
            'filePrefix' => 'Claims_Report',
            ])
        </div>
        <!-- Form to be toggled -->
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-12 col-lg-12 pt-4 d-flex flex-column">
                                <label for="grouptype">{{__('messages.claims_report.group_report_by')}}</label>
                               <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height: 3.5rem;" name="grouptype" id="grouptype" onchange="toggleDateFields()">
                                <option value="insurance_company" {{ request('grouptype')=='insurance_company' ? 'selected' : ''}}>{{__('messages.claims_report.grouptype_insurance_company')}}</option>
                                <option value="client_name" {{ request('grouptype')=='client_name' ? 'selected' : ''}}>{{__('messages.claims_report.grouptype_client_name')}}</option>
                                <option value="policy_type" {{ request('grouptype')=='policy_type' ? 'selected' : ''}}>{{__('messages.claims_report.grouptype_policy_type')}}</option>
                                <option value="claim_status" {{ request('grouptype')=='claim_status' ? 'selected' : ''}}>{{__('messages.claims_report.grouptype_claim_status')}}</option>
                            </select>
                            </div>
                            <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                               <label for="insurance_company">{{__('messages.claims_report.insurance_company')}}</label>
                                <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height: 3.5rem;" name="insurance_company" id="insurance_company">
                                    <option value="">{{__('messages.claims_report.select_company')}}</option>
                            ...
                            <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                               <label for="policy_type">{{__('messages.claims_report.policy_type')}}</label>
                                <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height: 3.5rem;" name="policy_type" id="policy_type">
                                        <option value="">{{__('messages.claims_report.select_policy_type')}}</option>
                            ...
                            <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                           <label for="claim_status">{{__('messages.claims_report.claim_status')}}</label>
                        <select class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height: 3.5rem;" name="claim_status" id="claim_status">
                            <option value="">{{__('messages.claims_report.select_claim_status')}}</option>
                            <option value="under review">{{__('messages.claims_report.under_review')}}</option>
                            <option value="received">{{__('messages.claims_report.received')}}</option>
                            <option value="closed">{{__('messages.claims_report.closed')}}</option>
                        </select>
                            </div>
                            <div class="col-12 col-lg-4 pt-4 d-flex flex-column">
                                <button type="submit" class="btn rounded-1 text-white opacity-50 p-2" style="background-color: #EF7C00;">{{__('messages.claims_report.generate_report')}}</button>
        ...
        <div class="m-2 text-center">
            ...
            <h4>{{__('messages.claims_report.claims_by')}}
    @php
    switch(request('grouptype'))
    {
    case 'insurance_company':
    echo __('messages.claims_report.heading_insurance_company');
    break;
    case 'policy_type':
    echo __('messages.claims_report.heading_policy_type');
    break;
    case 'client_name':
    echo __('messages.claims_report.heading_client_name');
    break;
    case 'claim_status':
    echo __('messages.claims_report.heading_claim_status');
    break;
    default:
    echo __('messages.claims_report.heading_insurance_company');
    break;
...
<p>{{__('messages.claims_report.from')}} {{ request('issue_date') ?? __('messages.claims_report.no_date') }} {{__('messages.claims_report.to')}} {{ request('expiry_date') ?? __('messages.claims_report.no_date') }}</p>
        </div>
        <div class="body bg-white py-4 px-4 d-flex flex-column gap-3 pb-5">
            @if($policies)
            ...
            <table class="table table-striped " style="width:100%">
                ...
                <tbody>
                    ...
                    <!-- Totals Row -->
<tr>
    <td colspan="6"><strong>{{__('messages.claims_report.total')}}</strong></td>
...
@endforeach
@else
<h4 class="text-center">{{__('messages.claims_report.no_record')}}</h4>
@endif
        </div>
    </div>
    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
