...
<main class="flex-grow-1 pt-5">
    <div class="container-fluid">
        <div class="header d-flex justify-content-between p-3 py-2 align-items-center">
            <div class="text-white group-title fw-bold"> {{__('messages.currency.currency')}} </div>
            <button data-bs-toggle="modal" data-bs-target="#addcsvcurrency" class="btn pe-0">
            ...
            <table id="example" class="table table-striped " style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th width="20%">{{__('messages.currency.country')}}</th>
                        <th width="60%"> {{__('messages.currency.currency')}} </th>
                        <th width="20%"> {{__('messages.currency.abbreviation')}} </th>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex gap-3 justify-content-evenly align-items-center px-2">
                                <!-- <button class="btn p-0 m-0 editbtn" value="{{$value->id}}" data-val="{{$value->name}}" data-abbreviation="{{$value->abbreviation}}" data-bs-toggle="modal" data-bs-target="#editModal"> -->
                                <button
                                    class="btn p-0 m-0 editbtn"
                                    value="{{$value->id}}"
                                    data-val="{{$value->name}}"
    ...
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header text-white " style="background-color: #104E9E;">
                <h1 class="modal-title fs-5"> {{__('messages.currency.add_currency')}} </h1>
                <button data-bs-dismiss="modal" class="btn">
                    <img src="{{asset('img/icon-close.svg')}}" alt="">
                </button>
            </div>
            <form action="{{route('currency.create')}}" id="add" method="post">
                @csrf
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    <div>{{__('messages.currency.country')}}</div>
                                ...
                                <div class="col-12 col-lg-6 pt-2">
                                    <div> {{__('messages.currency.add_currency')}} </div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <input type="text" name="name" style="border: none;" maxlength="50" required>
                                    </div>
                                </div>
                                <div class="col-12 col-lg-6 pt-2">
                                    <div> {{__('messages.currency.abbreviation')}} </div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <input type="text" name="abbreviation" style="border: none;" maxlength="5" required>
                                    </div>
                                </div>
                            </div>
                            <div class="row pt-5">
...
<!-- START ADD CSV -->
<div class="modal fade " id="addcsvcurrency" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header text-white " style="background-color: #104E9E;">
                <h1 class="modal-title fs-5"> {{__('messages.currency.add_currency')}} </h1>
            ...
            <form action="{{route('currency.csv')}}" id="addcsv" method="post" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="row p-3" style="color: #92959A;">
                        <div class="container">
...
<div class="modal fade " id="editModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header text-white " style="background-color: #104E9E;">
                <h1 class="modal-title fs-5"> {{__('messages.currency.update_currency')}} </h1>
                <button data-bs-dismiss="modal" class="btn">
                    <img src="{{asset('img/icon-close.svg')}}" alt="">
                </button>
            </div>

            <form action="{{route('currency.update')}}" id="edit_cd" method="post">
                @method('PATCH')
                @csrf
                <div class="modal-body">
                    <div class="row p-3" style="color: #92959A;">
                        <div class="container">
                            <div class="row">
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    <div>{{__('messages.currency.country')}}</div>
                                    <select name="country_id" id="country_edit"
                                        class="form-select rounded-0 flex-grow-1 border-0 shadow1"
                                        style="height: 3.5rem;"
                                        required>
                                ...
                                <div class="col-12 col-lg-6 pt-2">
                                    <div>{{__('messages.currency.update_currency')}}</div>
                                ...
                                <div class="col-12 col-lg-6 pt-2">
                                    <div> {{__('messages.currency.abbreviation')}} </div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <input type="text" id="abbreviation_edit" name="abbreviation" style="border: none;" maxlength="5" required>
                                    </div>
                                </div>
                            </div>
...
<div class="modal fade " id="DeleteModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            ...
            <form action="{{route('currency.destroy')}}" method="post">
...
<script>
    ...
    //     // alert(cd_id+" =============== "+val);
    //     $("#abbreviation_edit").val(abbreviation);
    //     $("#name_edit").val(val);
    //     $("#cd_id").val(cd_id);
