...
<main class="flex-grow-1 pt-5">
    <div class="container-fluid">
        <div class="header d-flex justify-content-between p-3 py-2 align-items-center">
            <div class="text-white group-title fw-bold"> {{__('messages.insurance_period.insurance_period')}} </div>
            <button data-bs-toggle="modal" data-bs-target="#add_insurance_period" class="btn pe-0">
            ...
            <table id="example" class="table table-striped " style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th width="90%"> {{__('messages.insurance_period.insurance_period')}} </th>
                ...
                <tbody>
                    ...
                    $sortedInsurancePeriods = $insurance_period
                    ->filter(function ($value) {
                    return !$value->trashed();
                    })
                    ->sortBy(function ($value) use ($unitOrder) {

                    $name = strtolower(trim($value->name));

                    preg_match(
                    '/(\d+(?:\.\d+)?)\s*.*?\b(hour|hours|day|days|week|weeks|month|months|year|years)\b/i',
                    $name,
                    $matches
                    );

                    if (empty($matches)) {
                    return [999, PHP_FLOAT_MAX, $name];
                    }

                    $number = (float) $matches[1];
                    $unit = strtolower($matches[2]);

                    $unitRank = $unitOrder[$unit] ?? 999;
                    ...
                    @foreach($sortedInsurancePeriods as $key => $value)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{$value->name}}</td>
                        <td>
...
<!-- START ADD Age -->
<div class="modal fade " id="add_insurance_period" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header text-white " style="background-color: #104E9E;">
                <h1 class="modal-title fs-5"> {{__('messages.insurance_period.add_insurance_period')}} </h1>
                <button data-bs-dismiss="modal" class="btn">
                    <img src="{{asset('img/icon-close.svg')}}" alt="">
                </button>
            </div>
            <form action="{{route('insurance_period.create')}}" id="add" method="post">
                @csrf
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="col-12 col-lg-6 pt-2">
                                    <div> {{__('messages.insurance_period.add_insurance_period')}} </div>
...
<div class="modal fade " id="editModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header text-white " style="background-color: #104E9E;">
                <h1 class="modal-title fs-5"> {{__('messages.insurance_period.update_insurance_period')}} </h1>
                <button data-bs-dismiss="modal" class="btn">
                    <img src="{{asset('img/icon-close.svg')}}" alt="">
                </button>
            </div>

            <form action="{{route('insurance_period.update')}}" id="edit_cd" method="post">
                @method('PATCH')
                @csrf
                <div class="modal-body">
                    <div class="row p-3" style="color: #92959A;">
                        <div class="container">
                            <div class="row">
                                <div class="col-12 col-lg-6 pt-2">
                                    <div>{{__('messages.insurance_period.update_insurance_period')}}</div>
...
<div class="modal fade " id="DeleteModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            <div class="modal-header text-white " style="background-color: #104E9E;">
                <h1 class="modal-title fs-5"> {{__('messages.insurance_period.delete_insurance_period')}} </h1>
                <button data-bs-dismiss="modal" class="btn">
                    <img src="{{asset('img/icon-close.svg')}}" alt="">
                </button>
            </div>

            <form action="{{route('insurance_period.destroy')}}" method="post">
