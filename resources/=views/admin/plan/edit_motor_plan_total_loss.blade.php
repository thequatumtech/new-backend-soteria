        ...
        <div class="container-fluid">
            <div class="header d-flex justify-content-between p-3 py-2 align-items-center">
                <div class="text-white group-title fw-bold">
                    @if(isset($plan))
                        {{__('messages.plans.edit_total_loss_plan')}}
                    @else
                        {{__('messages.plans.add_total_loss_plan')}}
                    @endif
                </div>
            </div>
            <form action="{{route('motor_plan.save')}}" id="add_motor_plan" method="post" enctype="multipart/form-data">
                ...
                <input type="hidden" id="motor_plan_id" name="motor_plan_id" value="6">
                        ...
                        <div class="container">
                            ...
                            <div class="row">
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    ...
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <input type="date" name="start_date" id="start_date" min="{{\Carbon\Carbon::parse($plan->start_date)->format('Y-m-d')}}" style="border: none; font-weight: 330;" required  value="{{old('start_date', $plan->start_date ?? '')}}">
                                    </div>
                                </div>
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    <div>{{__('messages.plans.to_date')}}</div>
                                    <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                        <input type="date" name="end_date" id="end_date" min="{{\Carbon\Carbon::parse($plan->start_date)->addYears(1)->format('Y-m-d')}}" style="border: none; font-weight: 330;" required readonly value="{{old('end_date', $plan->end_date ?? '')}}">
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    ...
                                    <select name="restricted_country_ids[]" class="form-select rounded-0 flex-grow-1 border-0 shadow1 select2" style="height: 3.5rem;" required multiple>
                                        @foreach($countries as $single)
                                            <option value="{{$single->id}}" {{in_array($single->id,old('restricted_country_ids', json_decode($plan->restricted_country_ids))) ? 'selected' : ''}}>{{$single->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    ...
                                    <select name="restricted_city_ids[]" class="form-select rounded-0 flex-grow-1 border-0 shadow1 select2" style="height: 3.5rem;" required multiple>
                                        @foreach($cities as $single)
                                            <option value="{{$single->id}}" {{in_array($single->id,old('restricted_city_ids', json_decode($plan->restricted_city_ids))) ? 'selected' : ''}}>{{$single->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    ...
                                    <select name="restricted_district_ids[]" class="form-select rounded-0 flex-grow-1 border-0 shadow1 select2" style="height: 3.5rem;" required multiple>
                                        @foreach($districts as $single)
                                            <option value="{{$single->id}}" {{in_array($single->id,old('restricted_district_ids', json_decode($plan->restricted_district_ids))) ? 'selected' : ''}}>{{$single->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    ...
                                    <select name="restricted_age_ids[]" class="form-select rounded-0 flex-grow-1 border-0 shadow1 select2" style="height: 3.5rem;" required multiple>
                                        @foreach($ages as $single)
                                            <option value="{{$single->id}}" {{in_array($single->id,old('restricted_age_ids', json_decode($plan->restricted_age_ids))) ? 'selected' : ''}}>{{$single->age}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    ...
                                    <select name="restricted_vehicle_type_ids[]" class="form-select rounded-0 flex-grow-1 border-0 shadow1 select2" style="height: 3.5rem;" required multiple>
                                        @foreach($vehicle_types as $single)
                                            <option value="{{$single->id}}" {{in_array($single->id,old('restricted_vehicle_type_ids', json_decode($plan->restricted_vehicle_type_ids))) ? 'selected' : ''}}>{{$single->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    ...
                                    <select name="restricted_vehicle_brand_ids[]" class="form-select rounded-0 flex-grow-1 border-0 shadow1 select2" style="height: 3.5rem;" required multiple>
                                        @foreach($vehicle_brands as $single)
                                            <option value="{{$single->id}}" {{in_array($single->id,old('restricted_vehicle_brand_ids', json_decode($plan->restricted_vehicle_brand_ids))) ? 'selected' : ''}}>{{$single->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    ...
                                    <select name="restricted_vehicle_category_ids[]" class="form-select rounded-0 flex-grow-1 border-0 shadow1 select2" style="height: 3.5rem;" required multiple>
                                        @foreach($vehicle_categories as $single)
                                            <option value="{{$single->id}}" {{in_array($single->id,old('restricted_vehicle_category_ids', json_decode($plan->restricted_vehicle_category_ids))) ? 'selected' : ''}}>{{$single->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    ...
                                    <select name="restricted_engine_type_ids[]" class="form-select rounded-0 flex-grow-1 border-0 shadow1 select2" style="height: 3.5rem;" required multiple>
                                        @foreach($engine_types as $single)
                                            <option value="{{$single->id}}" {{in_array($single->id,old('restricted_engine_type_ids', json_decode($plan->restricted_engine_type_ids))) ? 'selected' : ''}}>{{$single->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-12 col-lg-6 pt-4 d-flex flex-column">
                                    ...
                                    <select name="claim_deductible_ids[]" class="form-select rounded-0 flex-grow-1 border-0 shadow1 select2" style="height: 3.5rem;" required multiple>
                                        @foreach($claim_deductibles as $single)
                                            <option value="{{$single->id}}" {{in_array($single->id,old('claim_deductible_ids', json_decode($plan->claim_deductible_ids))) ? 'selected' : ''}}>{{$single->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <h3 class="pt-5">{{__('messages.plans.total_loss_schedule')}}</h3>
                            <table class="table-bordered dynamic-table" style="width: 100%;">
                                <thead>
                                <tr>
                            ...
                            <table class="table-bordered" style="width: 100%;">
                                ...
                                <tr>
                                    <td>
                                        <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                            <input type="text" name="fees" style="border: none;" value="{{old('fees', $plan->fees->fees)}}" required>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                            <input type="text" name="stamps" style="border: none;" value="{{old('stamps', $plan->fees->stamps)}}" required>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                            <input type="text" name="sales_tax" style="border: none;" value="{{old('sales_tax', $plan->fees->sales_tax)}}" required>
                                        </div>
                                    </td>
                                </tr>
                                </tbody>
                            ...
                            <table class="table-bordered" style="width: 100%;">
                                ...
                                <tr>
                                    <td>
                                        <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                            <input type="text" name="net_premium" style="border: none;" value="{{old('fees', $plan->fees->net_premium)}}" required>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                            <input type="text" name="commission_percentage" style="border: none;" value="{{old('fees', $plan->fees->commission_percentage)}}" required>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="shadow1 p-2" style="padding-bottom: 2.5rem;">
                                            <input type="text" name="commission_amount" style="border: none;" value="{{old('fees', $plan->fees->commission_amount)}}" required>
                                        </div>
                                    </td>
                                </tr>
                                </tbody>
