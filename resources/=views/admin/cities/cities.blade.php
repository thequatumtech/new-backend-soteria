...
@section('content')
    <main class="flex-grow-1 pt-5">
        <div class="container-fluid">
            <div class="header d-flex justify-content-between p-3 py-2 align-items-center">
                <div class="text-white group-title fw-bold"> {{__('messages.cities.cities')}} </div>
                <button data-bs-toggle="modal" data-bs-target="#addcsvcities" class="btn pe-0">
            ...
            <div class="d-flex p-3 py-2">
                <a href="{{route('cities.sample.csv')}}" class="ms-auto">
            ...
            <div class="body bg-white py-4 px-4 d-flex flex-column gap-3 pb-5">
                <table id="example" class="table table-striped " style="width:100%">
                    ...
                    <tr>
                        ...
                        <th width="45%"> {{__('messages.country.country')}} </th>
                        <th width="45%"> {{__('messages.cities.cities')}} </th>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-3 justify-content-evenly align-items-center px-2">
                                    <button class="btn p-0 m-0 editbtn" value="{{$value->id}}" data-val="{{$value->name}}" data-country_id="{{$value->country_id}}" data-bs-toggle="modal" data-bs-target="#editModal">
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <div class="modal-header text-white " style="background-color: #104E9E;">
                    <h1 class="modal-title fs-5"> {{__('messages.cities.add_cities')}} </h1>
                    <button data-bs-dismiss="modal" class="btn">
                        <img src="{{asset('img/icon-close.svg')}}" alt="">
                    </button>
                </div>
                <form action="{{route('cities.create')}}" id= "add" method="post">
                    @csrf
                            ...
                            <div class="container">
                                <div class="row">
                                    <div class="col-12 col-lg-6 pt-2">
                                        <div> {{__('messages.country.country')}} </div>
                                        <select name="country_id" class="form-select rounded-0 flex-grow-1 border-0 shadow1 valid" style="height: 3.5rem;" required>
                                            <option value="" disabled hidden>--Select--</option>
                                            @foreach($all_countries as $single)
                                                <option value="{{$single->id}}">{{$single->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-12 col-lg-6 pt-2">
                                        <div> {{__('messages.cities.add_cities')}} </div>
    ...
    <!-- START ADD CSV -->
    <div class="modal fade " id="addcsvcities" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <div class="modal-header text-white " style="background-color: #104E9E;">
                    <h1 class="modal-title fs-5"> {{__('messages.cities.add_cities')}} </h1>
                ...
                <form action="{{route('cities.csv')}}" id= "addcsv" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row p-3" style="color: #92959A;">
                            <div class="container">
    ...
    <div class="modal fade " id="editModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <div class="modal-header text-white " style="background-color: #104E9E;">
                    <h1 class="modal-title fs-5"> {{__('messages.cities.update_cities')}} </h1>
                    <button data-bs-dismiss="modal" class="btn">
                        <img src="{{asset('img/icon-close.svg')}}" alt="">
                    </button>
                </div>

                <form action="{{route('cities.update')}}" id="edit_cd" method="post">
                    @method('PATCH')
                    @csrf
                    <div class="modal-body">
                        <div class="row p-3" style="color: #92959A;">
                            <div class="container">
                                <div class="row">
                                    <div class="col-12 col-lg-6 pt-2">
                                        <div> {{__('messages.country.country')}} </div>
                                        <select name="country_id" id="edit_country_id" class="form-select rounded-0 flex-grow-1 border-0 shadow1" style="height: 3.5rem;" required>
                                            <option value="" disabled hidden>--Select--</option>
                                            @foreach($all_countries as $single)
                                                <option value="{{$single->id}}">{{$single->name}}</option>
                                            @endforeach
                                    ...
                                    <div class="col-12 col-lg-6 pt-2">
                                        <div>{{__('messages.cities.update_cities')}}</div>
    ...
    <div class="modal fade " id="DeleteModal" tabindex="-1" aria-labelledby="ModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <div class="modal-header text-white " style="background-color: #104E9E;">
                    <h1 class="modal-title fs-5"> {{__('messages.cities.delete_cities')}} </h1>
                    <button data-bs-dismiss="modal" class="btn">
                        <img src="{{asset('img/icon-close.svg')}}" alt="">
                    </button>
                </div>
...
@section('script')
    ...
    <script>
        ...
        $(document).on('click', '.editbtn', function() {
                ...
                // alert(cd_id+" =============== "+val);
            $('#edit_country_id').val(country_id).change();
                $("#name_edit").val(val);
                $("#cd_id").val(cd_id);
